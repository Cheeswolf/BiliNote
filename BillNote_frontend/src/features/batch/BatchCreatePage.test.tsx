...
it.each([false, true])('keeps actions outside scrollable content and clear of the desktop health overlay (Tauri: %s)', async isTauri => {
  ...
  // jsdom has no layout engine: guard the height/overflow contract explicitly.
  expect(main.classList.contains('min-h-0')).toBe(true)
  expect(main.classList.contains('overflow-y-auto')).toBe(true)
  expect(main.parentElement?.classList.contains('h-dvh')).toBe(true)
  ...
  expect(actions.contains(next)).toBe(true)
  await userEvent.click(next)
  await userEvent.click(screen.getByRole('checkbox', { name: '启用视频理解' }))
  fireEvent.change(screen.getByLabelText('备注'), { target: { value: '保留完整课程示例\n'.repeat(100) } })
  const start = screen.getByRole('button', { name: '开始生成 100 条笔记' }) as HTMLButtonElement
  expect(screen.getByRole('contentinfo', { name: '创建操作' }).contains(start)).toBe(true)
  expect(start.form?.contains(screen.getByLabelText('批次名称'))).toBe(true)
  expect(screen.getByRole('main').contains(start)).toBe(false)
  await userEvent.click(screen.getByRole('button', { name: '上一步' }))
  expect((screen.getByRole('checkbox', { name: '选择 课程 · P100' }) as HTMLInputElement).checked).toBe(true)
  await userEvent.click(screen.getByRole('button', { name: '下一步' }))
  expect((screen.getByLabelText('备注') as HTMLTextAreaElement).value).toContain('保留完整课程示例')
  await userEvent.click(screen.getByRole('button', { name: '开始生成 100 条笔记' }))
  await screen.findByText('批次已创建')
  expect(vi.mocked(submitBatch).mock.calls[0][0].items).toHaveLength(100)
})
