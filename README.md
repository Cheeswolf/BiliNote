...
> ## 🚧 Batch Video Notes Extension
>
> 本仓库 Fork 自 [JefferyHcool/BiliNote](https://github.com/JefferyHcool/BiliNote)。
>
> 当前 `feature/batch-video-notes` 分支在原 BiliNote 基础上增加了面向课程与多视频场景的**批量笔记生成工作流**。
>
> ### 本分支新增功能
>
> * 📚 **批量视频笔记生成**：一次提交多个视频或 Bilibili 分集任务
> * 🔍 **批量预览与分集解析**：在任务创建前解析视频及分集信息
> * 🗂️ **批量任务中心**：统一查看任务列表、单任务状态与整体进度
> * 🔄 **任务恢复机制**：应用重启或页面重新加载后可恢复批量任务状态
> * ♻️ **失败任务重试与重新生成**
> * 💾 **任务与笔记持久化**：批量任务、子任务和最终笔记结果独立保存
> * 🧩 **任务工作区隔离**：避免并行/连续任务之间的文件与状态互相污染
> * 🔐 **敏感配置清洗**：批量任务持久化时过滤 API Key、Token 等敏感设置
> * 🧪 **前后端自动化测试**：覆盖批量创建、轮询、恢复、任务队列与结果保存等核心流程
>
> ### 开发状态
>
> 当前功能仍处于独立功能分支开发阶段：
>
> `feature/batch-video-notes`
>
> 上游 BiliNote 的原始 README、部署说明与项目信息保留如下。
>
> ---
